...
function getLongestDinosaur(dinosaurs) {
  if(dinosaurs === undefined || dinosaurs.length === 0) {
    return {}
  }
  const metersToFeet = (meters) => {
    return meters * 3.28
  }
  const tallestDinosaur = dinosaurs.reduce((accumulator, current) => {
      if(current.lengthInMeters > accumulator.lengthInMeters) {
        return current
      } else {
        return accumulator
      }
  ...
  return {
    [tallestDinosaur.name]: metersToFeet(tallestDinosaur.lengthInMeters)
  }
}
...
function getDinosaursAliveMya(dinosaurs, mya, key) {
  const filteredDinos = dinosaurs.filter(dino => dino.mya.includes(mya) || dino.mya.includes(mya - 1))
  console.log(filteredDinos)
  // if(Objects.keys(filteredDinos).includes(key)) {
  //   return filteredDinos.name
  // }
}
